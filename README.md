...
The goal is simple: users should be able to upload or paste a document, understand what it means, see what matters, and know what they need to do next.

Example:
A user uploads or pastes a bill. LifePilot detects the payment deadline, amount, reference number, and prepares a reminder.

---

## Current Status

LifePilot is currently in an MVP development phase and is also a DevOps and Cloud Engineering portfolio project.

The web app already supports:

- login flow with mock/local development mode
- Cognito-ready authentication structure
- document upload UI
- TXT analysis in the browser
- text-based PDF extraction when readable text exists
- Smart Brain summaries for uploaded or pasted content
- server-side OpenAI analysis when configured
- deterministic fallback analysis when OpenAI is unavailable
...
Some data is still stored locally in the browser during development.
The AWS backend is prepared, but not fully deployed and validated yet.
...
The following parts are prepared or planned, but not fully production-ready:

- production AWS deployment
- durable AWS document storage
- deployed DynamoDB persistence for all user data
- private S3 document storage validation
- cross-device sync
...
LifePilot is planned as an AWS-first product.
...
LifePilot demonstrates practical work with:
